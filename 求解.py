            ...
            for i in range(self.num_plots):
                ...
                for s in range(2):
                    ...
                    for j in range(self.num_crops_per_plot):
                        ...
                        # 计算该作物的总面积
                        total_yield = area 
                        sellable_yield = min(total_yield, self.xiaoliang[j]*2 - accumulated_yield[j])
                            # 计算销售收入
    ...
    def crossover(self, parent1, parent2):
        ...
        for plot_index in range(self.num_plots):
            ...
            if category in [0, 1, 2]:  # 前三类地块
                crossover_point = np.random.randint(0, self.num_years)
                # 对每个地块在分类范围内进行交叉
                offspring1[:, plot_index] = np.copy(parent2[:, plot_index])
                offspring2[:, plot_index] = np.copy(parent1[:, plot_index])
                
                # 交换交叉点后的基因
                offspring1[crossover_point:, plot_index] = parent2[crossover_point:, plot_index]
                offspring2[crossover_point:, plot_index] = parent1[crossover_point:, plot_index]
                ...
                for s in range(2):
                    for j in range(self.num_crops_per_plot):
                        if np.random.rand() < mutation_rate:
                            # 随机调整作物面积
                            individual[t, i, j, s] = np.random.rand()
        return individual
